...
Do not treat this list as some outstanding problems or vulnerabilities on npm, Inc. side, it's closer to a wishlist of some hardening / transparency moments that would improve the overall ecosystem security. Also, good security is hard: some entries here require significant amount of work (some don't). The fact that they are not implemented yet is not a bug, vulnerability, or oversight, but keeping an open list of such moment still looks like a good idea to me. Also note that not all of this wishlist is targeted at npm, some of these could be implemented on the community side.
